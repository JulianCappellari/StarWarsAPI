...
describe('GET /api/planets', () => {
    ...
    it('debería devolver una lista de planetas', async () => {
        PlanetService_1.PlanetService.getPlanets.mockResolvedValue({
            total: 2,
            currentPage: 1,
            totalPages: 1,
    ...
    it('debería devolver un error 500 en caso de error del servidor', async () => {
        PlanetService_1.PlanetService.getPlanets.mockRejectedValue(new Error('Error simulado'));
        const response = await (0, supertest_1.default)(app).get('/api/planets').set('Accept', 'application/json');
